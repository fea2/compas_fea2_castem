            ...
            for element_type, sections in element_types.items():
                for section, elements in sections.items():
                    ...
                    for element in elements:
                        part_data.append('\nE{0} = MANU {1} N{2} ;'.format(element.jobdata()[0], element_type, element.jobdata()[1])) #la gestion des N devant les num de noeud n'est pas propre
                    ...
                    part_data.append('MOD{0}{1} = MODE MAIL{0}{1} {2} {3};'.format(obj.input_key, section.input_key, section.material.jobdata()[0], implementation))
                    part_data.append('MAT{0}{1} = MATE MOD{0}{1} {2} ;'.format(obj.input_key, section.input_key, section.material.jobdata()[1]))
...
class CastemDeformablePart(DeformablePart):
    ...
    def __init__(self, **kwargs):
        super(CastemDeformablePart, self).__init__(**kwargs)
